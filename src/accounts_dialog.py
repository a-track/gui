...
class AccountsDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.is_investment_checkbox.toggled.connect(self.toggle_valuation_visibility)
        new_account_layout.addWidget(self.is_investment_checkbox)
        
        self.valuation_combo = NoScrollComboBox()
        self.valuation_combo.addItems(['Total Value', 'Price/Qty', 'No Valuation'])
        ...
        # Disable Filters on ID (0) and Actions (9)
        ...
        header.setSectionResizeMode(3, QHeaderView.ResizeMode.ResizeToContents) # Company
        header.setSectionResizeMode(4, QHeaderView.ResizeMode.ResizeToContents) # Cur
        header.setSectionResizeMode(5, QHeaderView.ResizeMode.ResizeToContents) # Show
        header.setSectionResizeMode(6, QHeaderView.ResizeMode.ResizeToContents) # Active
        header.setSectionResizeMode(7, QHeaderView.ResizeMode.ResizeToContents) # Invest
        header.setSectionResizeMode(8, QHeaderView.ResizeMode.ResizeToContents) # Strat
    ...
    def load_accounts(self):
        try:
            ...
            # Filter out ghost rows and ID 0
            accounts = [
    ...
    def populate_table(self, accounts):
        ...
        # Filter valid accounts (must have ID and account name)
        valid_accounts = [a for a in accounts if a.id and a.account and str(a.account).strip()]
        ...
        for row, account in enumerate(valid_accounts):
            try:
                ...
                # 3: Company
                self.table.setItem(row, 3, QTableWidgetItem(account.company or ''))
                # 4: Currency
                ...
                # 5: Show in Balance
                self.table.setItem(row, 5, BooleanTableWidgetItem("Yes" if getattr(account, 'show_in_balance', True) else "No"))
                self.table.setCellWidget(row, 5, create_checkbox_widget(
                    getattr(account, 'show_in_balance', True), 'toggle_show_in_balance', 'show_in_balance'
                ))
                
                # 6: Active
                self.table.setItem(row, 6, BooleanTableWidgetItem("Yes" if getattr(account, 'is_active', True) else "No"))
                self.table.setCellWidget(row, 6, create_checkbox_widget(
                    getattr(account, 'is_active', True), 'toggle_active', 'is_active'
                ))
                ...
                # Removed inv_cb.setProperty('row', row) as it is unsafe with sorting
                inv_cb.toggled.connect(self.toggle_investment)
                inv_layout.addWidget(inv_cb)
                invest_widget.setLayout(inv_layout)
                self.table.setItem(row, 7, BooleanTableWidgetItem("Yes" if getattr(account, 'is_investment', False) else "No"))
                ...
                strat_combo.addItems(['Total Value', 'Price/Qty', 'No Valuation'])
                
                current_strat = getattr(account, 'valuation_strategy', 'Total Value')
                if not current_strat and getattr(account, 'is_investment', False):
                     current_strat = 'Total Value' # Default
                elif not current_strat:
                     current_strat = 'Total Value' # Fallback for display
                
                strat_combo.setCurrentText(current_strat)
                strat_combo.setProperty('account_id', account.id)
                strat_combo.currentIndexChanged.connect(self.change_valuation_strategy)
                
                # Only enable if investment
                strat_combo.setEnabled(getattr(account, 'is_investment', False))
    ...
    def on_cell_changed(self, row, column):
        try:
            ...
            # Update fields
            if column == 1: current_account.account = new_value
            elif column == 2: current_account.type = new_value
            elif column == 3: current_account.company = new_value
            elif column == 4: current_account.currency = new_value
    ...
    # --- Checkbox/Combo Handlers ---
    
    def toggle_show_in_balance(self, checked):
        sender = self.sender()
        if not sender: return
        self.budget_app.update_account_show_in_balance(sender.property('account_id'), checked)
        
        # Update backing item for filter
        index = self.table.indexAt(sender.parentWidget().pos())
        if index.isValid():
            item = self.table.item(index.row(), 5)
            if item:
                item.setData(ExcelHeaderView.FILTER_DATA_ROLE, "Yes" if checked else "No")
                item.setText("")
    ...
    def toggle_active(self, checked):
        sender = self.sender()
        if not sender: return
        self.budget_app.update_account_active(sender.property('account_id'), checked)
    ...
    def toggle_investment(self, checked):
        ...
        # Find current row (safe against sorting)
        index = self.table.indexAt(sender.parentWidget().pos())
        ...
        if account:
            ...
            # Enable/Disable dropdown in table
            ...
            # Update backing items (Invest=Col 7, Strat=Col 8)
            item_inv = self.table.item(row, 7)
            if item_inv: 
                item_inv.setData(ExcelHeaderView.FILTER_DATA_ROLE, "Yes" if checked else "No")
                item_inv.setText("")
            
            item_strat = self.table.item(row, 8)
            if item_strat and checked: item_strat.setText(account.valuation_strategy)
    ...
    def change_valuation_strategy(self):
        ...
        if account:
            ...
            if index.isValid():
                ...
                if item: item.setText(new_strat)
    ...
    def add_account(self):
        ...
        if not account_name or not currency or not account_type:
            self.show_status('Name, Type and Currency are required', error=True)
            return
    ...
    def show_status(self, message, error=False):
        ...
        self.status_label.setStyleSheet('color: #f44336' if error else 'color: #4CAF50')
