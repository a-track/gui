from PyQt6.QtWidgets import (QHeaderView, QStyle, QMenu, QWidgetAction, 
                             QCheckBox, QVBoxLayout, QWidget, QLineEdit, 
                             QPushButton, QHBoxLayout, QLabel, QDateEdit, 
                             QApplication, QScrollArea, QDialog, QDoubleSpinBox,
                             QTableWidgetItem)
from PyQt6.QtCore import Qt, QRect, QPoint, pyqtSignal, QDate
from PyQt6.QtGui import QPainter, QIcon, QAction, QColor, QPalette, QBrush
...
class ExcelHeaderView(QHeaderView):
    ...
    # Custom Role for filter values (to allow hidden values different from display text)
    ...
    def __init__(self, table, parent=None):
        ...
        # {col: {
        #    'hidden_values': set(), 
        #    'date_range': (min, max) or None,
        #    'number_filter': {'op': 'gt'|'lt', 'value': float}
        # }}
        self.filters = {} 
    ...
    def paintSection(self, painter, rect, logicalIndex):
        ...
        # Check Filter/Sort State
        is_filtered = self.is_column_filtered(logicalIndex)
        is_sorted = (self.table.horizontalHeader().sortIndicatorSection() == logicalIndex)
        ...
        if is_filtered:
            ...
            painter.drawText(icon_rect, Qt.AlignmentFlag.AlignCenter, "Y") # Funnel
    ...
    def mouseReleaseEvent(self, event):
        ...
        if icon_rect.contains(event.pos()):
            ...
            # Explicitly emit sectionClicked to ensure sorting triggers
            self.sectionClicked.emit(logicalIndex)
            super().mouseReleaseEvent(event)
            # User Request: Always scroll to top after sorting
    ...
    def show_filter_menu(self, col, global_pos):
        ...
        clear_action.triggered.connect(lambda: self.clear_filter_for_column(col))
        ...
        if col_type == 'number':
            ...
            def set_num_filter(op):
                ...
                if dlg.exec() == QDialog.DialogCode.Accepted and dlg.ok:
                     if col not in self.filters: self.filters[col] = {}
                     
                     # If setting a number filter, maybe clear hidden_values? 
                     # Or keep them? Usually they work together.
                     self.filters[col]['number_filter'] = {'op': op, 'value': dlg.value}
                     self.apply_filters()
                     self.viewport().update()
            
            num_menu.addAction("Greater Than...").triggered.connect(lambda: set_num_filter('gt'))
            num_menu.addAction("Less Than...").triggered.connect(lambda: set_num_filter('lt'))
        ...
        if col_type == 'date':
            ...
            def set_date_range():
                ...
                # Ideally pre-fill with current range?
                if dlg.exec() == QDialog.DialogCode.Accepted and dlg.ok:
                    if col not in self.filters: self.filters[col] = {}
                    
                    self.filters[col]['date_range'] = (dlg.start_date, dlg.end_date)
                    self.apply_filters()
                    self.viewport().update()
        ...
        # Get unique values for this column, respecting OTHER filters
        # We want to show only values that are pertinent given the other active filters.
        unique_values = set()
        
        # Helper to check if row matches all filters EXCEPT the current column
        def match_other_filters(r, current_col):
            # If no filters at all, quick return
            if not self.filters: return True
            
            for c, f_data in self.filters.items():
                if c == current_col: continue # Ignore self
                
                item = self.table.item(r, c)
                if not item: return False # Should not happen if well-formed, but safe? 
                # Actually if item is None, we treated it as visible in apply_filter?
                # No, standard logic might vary. Let's assume safe to skip if None.
                ...
                if 'number_filter' in f_data:
                    ...
                    try:
                        clean = val.split(' ')[0].replace("'", "").replace(",", "")
                        num = float(clean)
                        if nf['op'] == 'gt' and not (num > nf['value']): return False
                        elif nf['op'] == 'lt' and not (num < nf['value']): return False
                    except:
                        pass # formatting error -> pass or fail? strict: fail.
                ...
                if 'date_range' in f_data:
                    ...
                    try:
                        ...
                        if d.isValid():
                            if d < dr[0] or d > dr[1]: return False
                    except:
                        pass
        ...
        for row in range(self.table.rowCount()):
            if self.table.isRowHidden(row):
                # Optimization: If row is hidden, is it hidden by THIS col or OTHER col?
                # If hidden by OTHER col, we ignore it.
                # If hidden by THIS col (and others pass), we Include it (as unchecked).
                # But to know WHY it is hidden, we must re-evaluate logic.
                pass
            
            # More robust: ALWAYS check logic.
        ...
        # select_all_cb.setChecked(True)  <-- REMOVED: Managed by logic above
        ...
        ok_btn.setStyleSheet("background-color: #2196F3; color: white; border: none; padding: 5px 15px; border-radius: 3px;")
        cancel_btn.setStyleSheet("background-color: #f0f0f0; border: none; padding: 5px 15px; border-radius: 3px;")
    ...
    def apply_filters(self):
        # We need to know the Role ID for total row. 
        # It's defined in transactions_dialog, but we don't want cyclic imports. 
        # We can just use the int value: Qt.ItemDataRole.UserRole + 1
        TOTAL_ROW_ROLE = Qt.ItemDataRole.UserRole + 1

        for row in range(self.table.rowCount()):
            should_hide = False
            
            # CHECK FOR TOTAL ROW EXEMPTION
            # We check the first column item for the flag
            item0 = self.table.item(row, 0)
            if item0 and item0.data(TOTAL_ROW_ROLE):
                # This is a Total Row, NEVER hide it.
            ...
            for col, f_data in self.filters.items():
                ...
                if 'number_filter' in f_data:
                    ...
                    try:
                        ...
                        pass # Can't parse, don't hide? Or hide? Let's ignore.
                
                # Date Filter
                if 'date_range' in f_data:
                    dr = f_data['date_range']
                    # We need the date object. 
                    # Usually QTableWidgetItem text is YYYY-MM-DD
                    try:
                        d = QDate.fromString(val, "yyyy-MM-dd")
                        if d.isValid():
                             if d < dr[0] or d > dr[1]:
...
class BooleanTableWidgetItem(QTableWidgetItem):
    ...
    def __init__(self, value, display_text=""):
        super().__init__(display_text) # Typically empty string to hide text
