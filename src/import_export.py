...
class DataManager:
    ...
    def _validate_workbook(self, workbook):
        ...
        # Basic column validation could go here
        # For now, we rely on the schema match during insert.
        # But we can check for duplicate IDs in accounts/categories within Python
        
        # Check Account IDs uniqueness
        ...
        # Transactions foreign key check (Accounts)
        # In a real heavy app this might be slow, but for local usage likely fine.
        ...
        if len(rows) > 0:
            ...
            for i, row in enumerate(rows[1:], start=2):
                ...
                if val is not None and val not in acc_ids: # None might be allowed for some cases? Based on schema 'account_id' is nullable?
                    # Schema says: account_id INTEGER, foreign key...
                    # If it's transfer, might be different logic, but standard valid account_id is required usually.
    ...
    def _import_sheet(self, conn, sheet, table_name):
        ...
        header = [c.value for c in rows[0]]
        # We construct an INSERT statement based on header columns
        # ensuring they match table columns.
    ...
    def _export_exchange_rates_matrix(self, conn, workbook):
        ...
        """
        # 1. Fetch all data
        data = conn.execute("SELECT date, currency, rate FROM exchange_rates ORDER BY date DESC").fetchall()
        
        # 2. Key currencies
        currencies = sorted(list(set(r[1] for r in data)))
        
        # 3. Create Pivot Data
        pivot_data = {} # {date: {currency: rate}}
        for row in data:
            date_val = row[0]
            curr = row[1]
            rate = row[2]
    ...
    def _import_exchange_rates_matrix(self, conn, workbook):
        ...
        full_data = [] # List of (date, currency, rate)
        ...
        for row in rows[1:]:
            ...
            # Formats might be 'yyyy-mm-dd' string or datetime object
            if isinstance(date_val, datetime):
                date_str = date_val.strftime('%Y-%m-%d')
            else:
                date_str = str(date_val).split(" ")[0] # basic clean
            ...
            for i, cell in enumerate(row[1:]):
                ...
                if rate is not None and isinstance(rate, (int, float)) and rate > 0:
                     # Add to list
                     # We need columns (date, currency, rate) matching DB schema order if possible
                     # DB schema: id, date, currency, rate.
                     # We will insert (date, currency, rate) and let ID auto-increment or be handled by logic
                     full_data.append((date_str, currency, float(rate)))
        
        # Bulk Insert
        # We need to manage IDs manually as per our 'add_exchange_rate' logic or just insert?
        # The 'exchange_rates' table has an ID PK.
        # We can use our bulk method logic or direct SQL.
        # Direct SQL is faster here.
        
        # First get Max ID
        res = conn.execute("SELECT COALESCE(MAX(id), 0) FROM exchange_rates").fetchone()
        next_id = (res[0] if res else 0) + 1
        
        # Prepare batch with IDs
        final_batch = []
        for item in full_data:
             final_batch.append((next_id, item[0], item[1], item[2]))
             next_id += 1
             
        conn.executemany("INSERT INTO exchange_rates (id, date, currency, rate) VALUES (?, ?, ?, ?)", final_batch)
    ...
    def _export_investment_valuations_matrix(self, conn, workbook):
        ...
        # 2. Key Accounts (Name + ID)
        # We need ID to map back correctly on import if names duplicate (unlikely but safe)
        # Format: "Name_ID"
        accounts_map = {} # {id: "Name_ID"}
        for r in data:
            acc_name = r[1]
            acc_id = r[2]
        ...
        # Token format is "Name_ID"
        unique_tokens = sorted(list(set(accounts_map.values())), key=lambda x: x.rsplit('_', 1)[0].lower())
        
        # 3. Pivot
        pivot_data = {} # {date: {token: value}}
        for r in data:
            date_val = r[0]
            acc_id = r[2]
            val = r[3]
    ...
    def _import_investment_valuations_matrix(self, conn, workbook):
        ...
        # Parse Account IDs from headers: "Name_ID"
        # If ID is missing, we might need lookup by name, but export includes ID.
        # Importer assumes strict format from exporter or template.
        token_to_id = {}
        valid_indices = []
        ...
        full_data = [] # (date, account_id, value)
        ...
        # Bulk Insert
        res = conn.execute("SELECT COALESCE(MAX(id), 0) FROM investment_valuations").fetchone()
        next_id = (res[0] if res else 0) + 1
        ...
        conn.executemany("INSERT INTO investment_valuations (id, date, account_id, value) VALUES (?, ?, ?, ?)", final_batch)
    ...
    def generate_template(self, file_path):
        ...
        try:
            ...
            sheet.append(['id', 'account', 'type', 'company', 'currency', 'is_active', 'show_in_balance', 'is_investment', 'valuation_strategy'])
            sheet.append([0, 'Opening Balance', 'System', None, 'CHF', 1, 0, 0, None]) 
            sheet.append([1, 'Main Bank Account', 'Bank', 'UBS', 'CHF', 1, 1, 0, None])
            sheet.append([2, 'Cash Wallet', 'Cash', None, 'CHF', 1, 1, 0, None])
            sheet.append([3, 'Savings', 'Bank', 'Raiffeisen', 'CHF', 1, 1, 0, None])
            sheet.append([4, 'Global ETF Portfolio', 'Asset', 'Interactive Brokers', 'USD', 1, 1, 1, 'Total Value'])
            sheet.append([5, 'McDonalds Stock', 'Asset', 'eToro', 'USD', 1, 1, 1, 'Price/Qty'])
            ...
            sheet.append([current_id, '2025-01-01', 'transfer', 15000.00, 0, None, 'Opening Balance', 'Initial Savings', 1, 15000.00, None, None]); current_id += 1
            sheet.append([current_id, '2025-01-01', 'transfer', 200.00, 0, None, 'Opening Balance', 'Cash on hand', 2, 200.00, None, None]); current_id += 1
            sheet.append([current_id, '2025-01-01', 'transfer', 50000.00, 0, None, 'Opening Balance', 'Life Savings', 3, 50000.00, None, None]); current_id += 1
            
            sheet.append([current_id, '2025-01-02', 'transfer', 2500.00, 1, None, 'eToro', 'Buy McDonalds (10x)', 5, 2800.00, 10.0, None]); current_id += 1
            
            months = ['2025-01', '2025-02', '2025-03']
            
            for month in months:
                sheet.append([current_id, f'{month}-25', 'income', 6500.00, 1, 8, 'Employer Inc.', 'Monthly Salary', None, None, None, None]); current_id += 1
                
                sheet.append([current_id, f'{month}-01', 'expense', 1800.00, 1, 1, 'Landlord', 'Rent', None, None, None, None]); current_id += 1
                sheet.append([current_id, f'{month}-02', 'expense', 350.00, 1, 6, 'Insurance Co.', 'Health Insurance', None, None, None, None]); current_id += 1
                sheet.append([current_id, f'{month}-05', 'expense', 80.00, 1, 5, 'SBB', 'Monthly Pass', None, None, None, None]); current_id += 1
                
                sheet.append([current_id, f'{month}-06', 'transfer', 400.00, 1, None, 'ATM', 'Cash Withdrawal', 2, 400.00, None, None]); current_id += 1
                
                sheet.append([current_id, f'{month}-08', 'expense', 120.00, 1, 3, 'Coop', 'Weekly Groceries', None, None, None, None]); current_id += 1
                sheet.append([current_id, f'{month}-12', 'expense', 85.00, 1, 4, 'Restaurant', 'Dinner with friends', None, None, None, None]); current_id += 1
                
                sheet.append([current_id, f'{month}-15', 'expense', 200.00, 2, 3, 'Migros', 'Groceries (Cash)', None, None, None, None]); current_id += 1
                sheet.append([current_id, f'{month}-20', 'expense', 45.00, 2, 7, 'Cinema', 'Movie Night (Cash)', None, None, None, None]); current_id += 1
                
                sheet.append([current_id, f'{month}-26', 'transfer', 1000.00, 1, None, 'Self', 'Monthly Savings', 3, 1000.00, None, None]); current_id += 1
                
                sheet.append([current_id, f'{month}-28', 'transfer', 2000.00, 1, None, 'IBKR', 'Monthly ETF Buy', 4, 2200.00, 22.5, None]); current_id += 1 

            sheet.append([current_id, '2025-03-15', 'income', 50.00, 4, 9, 'Vanguard', 'Quarterly Dividend', None, None, None, None]); current_id += 1
            sheet.append([current_id, '2025-03-20', 'income', 15.00, 5, 9, 'McDonalds', 'Quarterly Dividend', None, None, None, None]); current_id += 1
            ...
            sheet = workbook.create_sheet('investment_valuations')
            sheet.append(['date', 'Global ETF Portfolio_4', 'McDonalds Stock_5'])
    ...
    def load_sample_data(self):
        ...
        try:
            ...
            success, msg = self.import_from_excel(path, progress, skip_backup=True)
