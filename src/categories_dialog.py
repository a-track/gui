...
class CategoriesDialog(QDialog):
    def __init__(self, budget_app, parent=None):
        ...
        self.table.setHorizontalHeaderLabels(['ID', 'Category Type', 'Category', 'Sub Category', 'Delete'])
        ...
        # Disable filter for 'Delete' button column (4) and ID (0)
    ...
    def load_categories(self):
        try:
            ...
            self.table.setColumnWidth(1, max(100, self.table.columnWidth(1))) # Type
            self.table.setColumnWidth(2, max(120, self.table.columnWidth(2))) # Parent
            self.table.setColumnWidth(3, max(120, self.table.columnWidth(3))) # Sub
            self.table.setColumnWidth(4, max(50, self.table.columnWidth(4)))  # Delete
    ...
    def populate_table(self, categories):
        ...
        # Filter valid categories (must have ID and category name)
        valid_categories = [c for c in categories if c.id and c.category and str(c.category).strip()]
    ...
    def on_cell_changed(self, row, column):
        try:
            # Columns: 0=ID(NoEdit), 1=Type, 2=Parent, 3=Sub, 4=Delete
            ...
            # Get ID from Column 0 (Safe against sorting/filtering)
            ...
            # Find the original sub_category name (required for models.update_category)
            # We iterate all to find it (robust)
            categories = self.budget_app.get_all_categories()
            orig_cat_obj = next((c for c in categories if c.id == cat_id), None)
            
            if not orig_cat_obj:
                self.show_status("Error: Category not found in DB", error=True)
                return
                
            # Current Sub Category Name (Key for update)
            # old_sub_category_key = orig_cat_obj.sub_category # Not needed anymore
            
            kwargs = {}
            if column == 1: # Type
                kwargs['new_type'] = new_value
            elif column == 2: # Parent Category
                kwargs['new_category'] = new_value
            elif column == 3: # Sub Category
            ...
            if success:
                ...
                self.show_status(f'Error updating category (possibly duplicate)', error=True)
                # Revert simple
                self.table.blockSignals(True)
                if column == 1: item.setText(orig_cat_obj.category_type)
                elif column == 2: item.setText(orig_cat_obj.category)
                elif column == 3: item.setText(orig_cat_obj.sub_category)
                self.table.blockSignals(False)
                
        except Exception as e:
            print(f"Error in on_cell_changed: {e}")
            self.show_status('Error updating category', error=True)
            # Attempt generic revert if possible
            try:
                self.table.blockSignals(True)
                categories = self.budget_app.get_all_categories()
                orig_cat = next((c for c in categories if c.id == cat_id), None)
                if orig_cat:
                     if column == 1: item.setText(orig_cat.category_type)
                     elif column == 2: item.setText(orig_cat.category)
                     elif column == 3: item.setText(orig_cat.sub_category)
                self.table.blockSignals(False)
            except:
                pass
    ...
    def delete_category(self):
        try:
            ...
            # Resolve ID to name for method call and display
            categories = self.budget_app.get_all_categories()
