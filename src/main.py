...
def create_placeholder_splash():
    # Programmatically create a splash image
    pixmap = QPixmap(500, 300)
    pixmap.fill(QColor(255, 255, 255))
    painter = QPainter(pixmap)
    painter.setRenderHint(QPainter.RenderHint.Antialiasing)
    
    # Draw fancy background (gradient-ish)
    painter.fillRect(0, 0, 500, 300, QColor(245, 247, 250))
    painter.fillRect(0, 0, 500, 10, QColor(33, 150, 243)) # Top bar
    
    # Text
    font = QFont("Segoe UI", 28, QFont.Weight.Bold)
    painter.setFont(font)
    painter.setPen(QColor(33, 150, 243))
    painter.drawText(pixmap.rect().adjusted(0, -20, 0, 0), Qt.AlignmentFlag.AlignCenter, "Budget Tracker")
    
    font_sub = QFont("Segoe UI", 12)
    painter.setFont(font_sub)
    painter.setPen(QColor(100, 100, 100))
    painter.drawText(pixmap.rect().adjusted(0, 60, 0, 0), Qt.AlignmentFlag.AlignCenter, "Loading your finances...")
    ...
    while True:
            ...
            if dialog.exec():
                ...
                if getattr(dialog, 'mode', 'new') == 'sample':
                    try:
                        ...
                        splash.showMessage("Generating sample data...", Qt.AlignmentFlag.AlignBottom | Qt.AlignmentFlag.AlignCenter, QColor("black"))
                        app.processEvents()
                        
                        success, msg = dm.load_sample_data()
                        
                        if not success:
                            QMessageBox.warning(None, "Sample Data Error", f"Could not load sample data: {msg}")
        ...
        try:
            ...
            try:
                ...
                QMessageBox.critical(None, "Application Error", f"Critical Error: {error_msg}")
            except:
                pass
            sys.exit(1)
