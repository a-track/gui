...
def check_vwrl():
    ...
    for i in range(5):
        try:
            ...
            print(f"Waiting for DB lock... ({i+1}/5)")
            time.sleep(1)
    ...
    # 1. Find VWRL
    res = conn.execute("SELECT id, account, valuation_strategy FROM accounts WHERE account LIKE '%VWRL%'").fetchone()
    if not res:
    ...
    rows = conn.execute("""
        SELECT date, description, amount, source_account_id, destination_account_id
        FROM transactions 
    ...
    for r in rows:
        print(f" - {r[0]}: {r[1]} Amt={r[2]} Src={r[3]} Dst={r[4]}")
